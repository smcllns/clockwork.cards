...
export default function BrushingCardFlat({ dob, name }: { dob: Date; name: string }) {
  ...
  return (
    <Slide id="4b-flat">
      <p className="text-xl sm:text-2xl leading-relaxed mb-12 font-medium text-(--text-primary)">
        If {name} spends <InlineStepper value={minutes} min={0} max={10} step={0.5} unit=" min" onChange={setMinutes} /> brushing teeth each
        morning and night &mdash; so <span className="font-bold font-(--font-stat) text-(--text-primary)">{minutes * 2} minutes</span> each
        day &mdash; that's over{" "}
        <span className="font-bold font-(--font-stat) text-(--text-primary)">{(brushStrokes / 1e6).toFixed(1)} million brush strokes*</span>
        so far!
      </p>
      <p className="text-lg leading-loose mb-8 text-(--text-secondary)">
