...
describe("expandBase", () => {
  test("age 8 base 10 — padded to 3 digits, two lines", () => {
    expect(expandBase(8, 10, 3)).toEqual([
      "8₁₀ = (0×10²) + (0×10¹) + (8×10⁰)",
      "= 0 + 0 + 8 = 8",
    ]);
  });
  ...
  test("age 9 base 10 — padded to 3 digits", () => {
    expect(expandBase(9, 10, 3)).toEqual([
      "9₁₀ = (0×10²) + (0×10¹) + (9×10⁰)",
      "= 0 + 0 + 9 = 9",
    ]);
  });
  ...
  test("age 10 base 10 — padded to 3 digits", () => {
    expect(expandBase(10, 10, 3)).toEqual([
      "10₁₀ = (0×10²) + (1×10¹) + (0×10⁰)",
      "= 0 + 10 + 0 = 10",
    ]);
  });
  ...
  test("age 10 base 2", () => {
    expect(expandBase(10, 2)).toEqual([
  ...
  test("age 12 base 10 — padded to 3 digits", () => {
    expect(expandBase(12, 10, 3)).toEqual([
      "12₁₀ = (0×10²) + (1×10¹) + (2×10⁰)",
      "= 0 + 10 + 2 = 12",
    ]);
  });
  ...
  test("age 12 base 2", () => {
    expect(expandBase(12, 2)).toEqual([
      "1100₂ = (1×2³) + (1×2²) + (0×2¹) + (0×2⁰)",
      "= 8 + 4 + 0 + 0 = 12",
    ]);
  });

  test("age 39 base 2 — 6 bits", () => {
    expect(expandBase(39, 2)).toEqual([
      "100111₂ = (1×2⁵) + (0×2⁴) + (0×2³) + (1×2²) + (1×2¹) + (1×2⁰)",
      "= 32 + 0 + 0 + 4 + 2 + 1 = 39",
    ]);
  });

  test("last line always ends with the number", () => {
    for (const age of [5, 6, 7, 8, 9, 10, 11, 12, 13, 15, 16, 20, 39]) {
      for (const base of [2, 10] as const) {
        const lines = expandBase(age, base, base === 10 ? 3 : 1);
        expect(lines[lines.length - 1]).toEndWith(`= ${age}`);
        const repr = base === 2 ? age.toString(2) : String(age);
        const sub = base === 10 ? "₁₀" : "₂";
        expect(lines[0]).toStartWith(`${repr}${sub} = `);
      }
    }
  });
});
