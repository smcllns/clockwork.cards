...
export function SpaceSection({ name, pronouns, shiny, space }: Props) {
  return (
    <PhotoSlide id="2" imgLight={imgLight} imgShiny={imgShiny} shiny={shiny}>
      <Intro className="mb-2">In {Math.round(space.lapsAroundSun)} laps around the sun, {name} has flown ...</Intro>
      <Stat>{((space.milesInSpace * space.k) / 1e9).toFixed(1)} billion {space.unit}</Stat>
      ...
      <Lede>{pronouns === "m" ? "He" : "She"}'s not just a kid, {pronouns === "m" ? "he" : "she"}'s an interstellar traveler!</Lede>
      <Body>
